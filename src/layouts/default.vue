...
function navActive(item: { to: string, exact?: boolean }) {
  ...
  if (query) {
    ...
    return route.path === path && Array.from(expected.entries()).every(([key, value]) => String(route.query[key] || '') === value)
  }
  if (item.exact)
...
.nav-icon[data-icon="calculator"]:before{content:"⊞"}.nav-icon[data-icon="document"]:before{content:"▤"}
</style>
